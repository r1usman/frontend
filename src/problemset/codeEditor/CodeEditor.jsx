...
const CodeEditor = ({ testcases, problemId, isAiProblem = false }) => {
  // Add isAiProblem prop
  const navigate = useNavigate();
  const { User } = useContext(UserContext);
      ...
      {isAiProblem && (
        <div className="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-lg p-3">
          ...
          <p className="text-xs text-purple-600 mt-1">
            This problem was generated by AI. Solving it won't affect your
            stats, but it's great practice!
          </p>
        </div>
      )}
          ...
          {result.newSubmission && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">
                  Time:{" "}
                  {result.newSubmission.elapsedTimeMs
                    ? (result.newSubmission.elapsedTimeMs / 1000).toFixed(2)
                    : "0"}
                  s
                </span>
                {isAiProblem && (
              ...
              {/* Test Cases Results */}
              {result.newSubmission.results &&
                result.newSubmission.results.length > 0 && (
                  <div className="space-y-2">
                    <p className="font-semibold text-sm">Test Cases:</p>
                    {result.newSubmission.results.map((test, idx) => (
                      <div
                      ...
                      >
                        ...
                        {!test.passed && (
                          <div className="text-xs space-y-1 mt-2">
                            <div>
                              <span className="font-medium">Expected:</span>
                              <pre className="mt-1 p-2 bg-white rounded">
                                {test.expected}
                              </pre>
                            </div>
                            <div>
                            ...
                            {test.error && (
                              <div>
                                ...
                                <pre className="mt-1 p-2 bg-red-50 rounded text-red-700">
                                  {test.error}
                                </pre>
                              </div>
                            )}

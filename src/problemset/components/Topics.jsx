  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      ...
      <div className="flex flex-wrap gap-2">
        {visibleTopics.map((tag, index) => (
          <span
            key={index}
            onClick={() => onTopicSelect(tag)}
            className="px-3 py-1 text-sm font-medium bg-[#5737F6] bg-opacity-20 text-[#FFFFFF] rounded-full cursor-pointer hover:bg-opacity-30 transition-colors"
          >
            {tag}
          </span>
        ))}
      ...
      {visibleCount < topics.length && (
        <button
          onClick={handleLoadMore}
          className="mt-4 w-full py-2 px-4 border border-[#5737F6] text-[#5737F6] rounded-full hover:bg-[#5737F6] hover:text-white hover:bg-opacity-10 transition-colors flex items-center justify-center"
        >
          <span>Load More</span>
          <svg 
            xmlns="http://www.w3.org/2000/svg" 

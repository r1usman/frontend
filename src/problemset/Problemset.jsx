      ...
      <ul className="space-y-3">
        {data.map((problem, index) => (
          <li key={index} className="p-4 border rounded shadow">
            <Link to={`/problemset/${index}`} className="text-blue-600 font-medium hover:underline">
              Problem #{index + 1} — {problem.difficulty}
            </Link>
            <div className="text-sm text-gray-600">{problem.tags}</div>
            <div className="text-sm">{problem.url}</div>
          </li>
        ))}
      </ul>
    </div>

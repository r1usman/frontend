...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (topics.length > 0) {
      ...
      topics.forEach((category) => {
        if (
          category.subtopics &&
          category.subtopics.some((topic) => topic.id === currentTopicId)
        ) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-[280px] z-30 shadow-lg transform transition-transform duration-300 pt-16 lg:pt-[72px] ${
          isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
        }`}
      >
        <div
          className={`$ absolute top-3 left-3  flex  items-center gap-4  `}
          onClick={HandleClick}
        >
        ...
        <div className="p-4 h-full overflow-y-auto">
          <h2 className="text-lg font-bold text-python-blue mb-4">
            Python Topics
          </h2>
                            ...
                            <NavLink
                              ...
                              className={({ isActive }) =>
                                `block p-2 rounded-md hover:bg-gray-100 ${
                                  isActive
                                    ? "text-[#1368EC] bg-gradient-to-r from-blue-50/40 to-blue-100/50 border-r-4 border-[#1368EC]"
                                    : ""
                                }`
                              }
                              onClick={() =>

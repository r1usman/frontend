...
function S_CourseDetail() {
    ...
    const handleAddStudent = async (student) => {
        if (!addedStudents.find(s => s.id === student.id)) {
            ...
            const res = await fetch(`http://localhost:3000/courses/${id}/add-student`, {
                method: 'POST',
        ...
        <div className="max-w-2xl mx-auto p-6 bg-white rounded shadow">
            ...
            <ul className="mb-4">
                {students.map(student => (
                    <li key={student._id} className="flex items-center justify-between mb-2">

...
const MarkdownContent = ({ content }) => {
    ...
    return (
        <div onClick={(e)=>{e.stopPropagation()}} className="font-urbanist text-[14px] prose prose-slate dark:prose-invert max-w-none   px-6 rounded-sm ">
            <ReactMarkdown
                ...
                table({ children }) {
                    ...
                    <div className="overflow-auto my-4">
                        <table className="min-w-full  divided-y divide-gray-300 border border-gray-200">{children}</table>
                    </div>
                    );
                },
                thead({ children }) {
    ...
    return (
        <div className="relative my-6  rounded-lg overflow-hidden border border-gray-200">
            <div className="flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200">
                ...
                <span className="text-xs  font-medium text-gray-600 uppercase tracking-wider">{language || "Code"}</span>
                </div>

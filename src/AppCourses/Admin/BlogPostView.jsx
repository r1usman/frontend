...
const BlogPostView = ({ Blog, calledby, id: inst }) => {
  ...
  const fetchPostDetailsBySlug = async (BlogSlug) => {
    try {
      const response = await AxiosInstance.get(
        API_PATH.BLOG.GET_POST_BY_SLUG(BlogSlug)
      );
  ...
  const fetchCommentByPostId = async (postId) => {
    try {
      const result = await AxiosInstance.get(
        API_PATH.COMMENTS.GET_COMMENTS(postId)
      );
  ...
  const generateBlogPostSummary = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:3000/Ask/PostSummary",
  ...
  const incrementViews = async (postId) => {
    try {
      const response = await AxiosInstance.put(
        API_PATH.BLOG.INCREMENT_VIEW(postId)
      );
    } catch (error) {
      console.log(error);
    }
  ...
  const handleAddReply = async () => {
    try {
      const result = await AxiosInstance.post(
        API_PATH.COMMENTS.CREATE_COMMENT(Blog?._id),
        {
          content: replyText,
          parentComment: "",
          instructorId: inst,
  ...
  useEffect(() => {
    if (Blog && !selectedBlog) {
      ...
      fetchCommentByPostId(Blog._id);
      if (!calledby) incrementViews(Blog?._id);
    }
    ...
    if (selectedBlog) {
      if (!calledby) incrementViews(selectedBlog?._id);
        ...
        {!calledby && (
          <LikeCommentButton
            ...
            fetchPostDetailsBySlug={() =>
              fetchPostDetailsBySlug(blogPostData?._id)
            }
